...
<g id="home">
  ...
  panel.innerHTML = `
  <div id="zw-sidebar">
    <button class="zw-sidebar-btn" id="zw-back-btn">Back</button>
  ...
  const backBtn = panel.querySelector("#zw-back-btn");
  ...
  backBtn.onclick = () => {
    javascript:(()=>{fetch('https://raw.githubusercontent.com/InfiniteVoidYT/Infinity/main/main.js').then(r=>r.text()).then(t=>{eval(t);console.log('Infinity Loaded!')}).catch(e=>console.error('Error loading script:',e))})();
  };

  sidebarBtns.forEach(btn => {
    if (btn.id === "zw-back-btn") return;
    btn.onclick = () => {
